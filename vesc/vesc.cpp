...
void vesc_rotation::duty_rotate(float power_ratio){
    ...
    // uint8_t converted_pwm_value_data[4] = {pwm_value >> 24, pwm_value >> 16, pwm_value >> 8, pwm_value & 0xff}; //edian?
    uint8_t converted_pwm_value_data[4]{};
    std::memcpy(converted_pwm_value_data, &pwm_value, 4);
...
void vesc_rotation::current_rotate(float power_ratio){
    ...
    uint8_t send_data[4]{};
    std::memcpy(send_data, &ratio_data, 4);
